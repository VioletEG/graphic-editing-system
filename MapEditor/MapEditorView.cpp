...
CPoint GLinMMPnt(-1, -1);//记录鼠标移动时的上-状态,用来擦除移动时的前一条线
...
State GCurShowState = SHOWSTATE_UNDEL;//显 示状态,默认为显示非删除状态
...
BEGIN_MESSAGE_MAP(CMapEditorView, CView)
	...
	ON_COMMAND(ID_FILE_SAVEPOINT, &CMapEditorView::OnFileSaveLine)
	...
	ON_COMMAND(ID_WINDOW__MOVE, &CMapEditorView::OnWindowMove)
	ON_COMMAND(ID_WINDOW_RESET, &CMapEditorView::OnWindowReset)
	ON_COMMAND(ID_WINDOW_POINT, &CMapEditorView::OnWindowShowPoint)
	ON_COMMAND(ID_WINDOW_LINE, &CMapEditorView::OnWindowShowLine)
	ON_COMMAND(ID_WINDOW_REGION, &CMapEditorView::OnWindowShowRegion)
	...
	ON_COMMAND(ID_POINT_SHOW_DELETE, &CMapEditorView::OnPointShowDelete)
	ON_COMMAND(ID_POINT_UNDELETE, &CMapEditorView::OnPointUndelete)
	ON_COMMAND(ID_POINT_MODIFY_PARPAMETER, &CMapEditorView::OnPointModifyParpameter)
	...
	ON_WM_DESTROY()
	ON_WM_MOUSEMOVE()
	ON_UPDATE_COMMAND_UI(ID_WINDOW_POINT, &CMapEditorView::OnUpdateWindowShowPoint)
	ON_UPDATE_COMMAND_UI(ID_WINDOW_LINE, &CMapEditorView::OnUpdateWindowShowLine)
	ON_UPDATE_COMMAND_UI(ID_WINDOW_REGION, &CMapEditorView::OnUpdateWindowShowRegion)
	ON_UPDATE_COMMAND_UI(ID_POINT_SHOW_DELETE, &CMapEditorView::OnUpdatePointShowDelete)
...
void CMapEditorView::OnDraw(CDC* /*pDC*/)
{
	...
	CClientDC dc(this);
	dc.FillSolidRect(0, 0, mrect.Width(), mrect.Height(), dc.GetBkColor()); // 用一单色填充一个矩形.
	dc.SetROP2(R2_NOTXORPEN);//设置绘图模式
	//ShowAl1Pnt(&dc, GPntTmpF, GPntNum);// 绘制显示所有点
	...
	//绘制显示所有区
	if (GShowReg)
	ShowAllReg(&dc, GRegTmpNdxF, GRegTmpDatF, GRegNum, GZoomOffset_x, GZoomOffset_y, GZoom, GCurShowState);//显示区
...
void CMapEditorView::OnRButtonUp(UINT /* nFlags */, CPoint point)
{
	...
	if (GRegFCreated) {
		switch (GCurOperState)
		{
			case OPERSTAE_INTPUT_REG ://当 前为造区操作状态
				if (GTReg.dotNum > 2)
				{
...
void CMapEditorView::OnFileNew()
{
	...
	if (!GRegFCreated) //判断线临时文件是否存在, 不存在则新建
	{
		GRegTmpNdxFName = dlg.m_add + CString("\\") + GRegTmpNdxFName; //临时线索引文件名
		GRegTmpDatFName = dlg.m_add + CString("\\") + GRegTmpDatFName; //临时线数据文件名
		if (GRegTmpNdxF->Open(GRegTmpNdxFName, CFile::modeCreate
			| CFile::modeReadWrite | CFile::typeBinary) && GRegTmpDatF->Open(GRegTmpDatFName,
				CFile::modeCreate | CFile::modeReadWrite | CFile::typeBinary))
		{
			GRegFCreated = true; //设置线临时文件新建成功标志值
...
void CMapEditorView::OnFileOpenPoint()
{
	...
	if (dlg.DoModal() == IDOK) // 弹出打开文件对话框让 用户指定要打开的文件
	{
		...
		if (GPntTmpFName != "tempPntF.dat") //如果临时点文件已经 创建则定位到文件开头
		{
...
void CMapEditorView::OnFileOpenLine()
{
	...
	if (dlg.DoModal() == IDOK)
	{
		GLinFCreated = false;
		int IsCreate = 0;
		GLinFName = dlg.m_ofn.lpstrFile;		CFile* LinF = new CFile();
		if (!LinF->Open(GLinFName, CFile::modeRead | CFile::typeBinary))
		{
			TRACE(_T("File could not be opened \n"));
			return;
		...
		if (2 == IsCreate)
			GLinFCreated = true;
		ReadLinPermanentFileToTemp(LinF, GLinTmpDatF, GLinTmpNdxF, GLinVer, GLinNum, GLinLNum, GLin.datOff); 		LinF->Close(); 		delete LinF;
		CString windowText = dlg.GetFileName() + "- MapEciitor";
		GetParent()->SetWindowTextW(windowText);
		this->InvalidateRect(NULL); //重绘窗口显示 
		GCurOperState = Noaction;
	}
...
void CMapEditorView::OnFileOpenRegion()
{
	...
	if (dlg.DoModal() == IDOK)
	{
		GRegFCreated = false;
		int IsCreate = 0;
		GRegFName = dlg.m_ofn.lpstrFile;		CFile* RegF = new CFile();
...
void CMapEditorView::OnFileSaveLine()
{
	...
	//已经存在临时文件
		...
		MessageBox(L"Eile have not been created. ", L"Message", MB_OK);
	}
	GCurOperState = Noaction; 
}
...
void CMapEditorView::OnFileSaveAsPoint()
{
	...
	GPntFName = CString("");//将永久文件名设置为空
	OnFileSavePoint();//调用该函数，永久文件名为空时自动弹出文件名对话框
	if (GPntFName == "")//若保存失败，则还原原永久文件名
	{
		GPntFName = tempFName;
	}
}
...
void CMapEditorView::OnFileSaveAsLine()
{
	...
	GLinFName = CString("");//将永久文件名设置为空
	OnFileSavePoint();//调用该函数，永久文件名为空时自动弹出文件名对话框
	if (GLinFName == "")//若保存失败，则还原原永久文件名
	{
		GLinFName = tempFName;
	}
...
void CMapEditorView::OnFileSaveAsRegion()
{
	...
	GRegFName = CString("");//将永久文件名设置为空
	OnFileSavePoint();//调用该函数，永久文件名为空时自动弹出文件名对话框
	if (GRegFName == "")//若保存失败，则还原原永久文件名
	{
		GRegFName = tempFName;
	}
}
...
void CMapEditorView::OnAppExit()
{
	...
	//调用父窗口CMainFrame的销毁窗口函数DestroyWindow ()
...
void CMapEditorView::OnPointCreate()
{
	// TODO: 在此添加命令处理程序代码
	if (GPntFCreated) {
		GCurOperState = OPERSTAE_INPUT_PNT; //设置为“造点"状态
...
void CMapEditorView::OnPointMove()
{
	...
	if (GPntFCreated)
	{
		GCurOperState = OPERSTAE_MOVE_PNT; // 设置操作状态(移动点)
		GCurShowState = SHOWSTATE_UNDEL;//设置当前为显示未删除状态
		this->Invalidate();
		GShowPnt = true;
		GShowReg = true;
...
void CMapEditorView::OnPointDelete()
{
	// TODO: 在此添加命令处理程序代码
	if (GPntFCreated) {
		GCurOperState = OPERSTAE_DELETE_PNT; //设置操作状态(删除点)
		GCurShowState = SHOWSTATE_UNDEL;//设置当前为显示未删除状态
		this->Invalidate();
		GShowPnt = true;
		GShowReg = true;
...
void CMapEditorView::OnPointShowDelete()
{
	// TODO: 在此添加命令处理程序代码
	...
	{
		GCurShowState = SHOWSTATE_UNDEL;//设置为 显示未删除状态
		GShowPnt = true;
		GShowLin = true;
		GShowReg = true;
	}
...
void CMapEditorView::OnPointModifyParpameter()
{
	// TODO: 在此添加命令处理程序代码
	if (GPntFCreated)
	{
...
{
	// TODO: 在此添加命令处理程序代码
	CLineParameterDlg dlg; // 参数设置的对话框
	dlg.m_Pattern = GLin.pattern;//型
...
void CMapEditorView::OnLineCreate()
{
	// TODO: 在此添加命令处理程序代码
	if (GLinFCreated) {
		GCurOperState = OPERSTAE_INPUT_LIN; // 当前为造线状态
		GCurShowState = SHOWSTATE_UNDEL;//设置当前为显示未删除状态
		this->Invalidate();
		GShowPnt = true;
		GShowReg = true;
		...
		MessageBox(L"TempFile have not been ereated.", L"Message", MB_OK);
	}
}
...
void CMapEditorView::OnLineMove()
{
	// TODO: 在此添加命令处理程序代码
	if (GLinFCreated) {
		GCurOperState = OPERSTAE_MOVE_LIN; // 当前为移动线操作状态
		GCurShowState = SHOWSTATE_UNDEL;//设置当前为显示未删除状态
		this->Invalidate();
		GShowPnt = true;
		GShowReg = true;
...
void CMapEditorView::OnLineDelete()
{
	// TODO: 在此添加命令处理程序代码
	if (GLinFCreated) {
		GCurOperState = OPERSTAE_DELETE_LIN;//当前状态为删除线
		GCurShowState = SHOWSTATE_UNDEL;//设置当前为显示未删除状态
		this->Invalidate();
		GShowPnt = true;
		GShowReg = true;
...
void CMapEditorView::OnLineLink()
{
	...
	if (GLinFCreated) {
		...
		GCurOperState = OPERSTATE_LINK_LIN;//设置 为连接线操作状态.
	}
	else {
...
void CMapEditorView::OnRegionCreate()
{
	...
	if (GRegFCreated)
	{
		GCurOperState = OPERSTAE_INTPUT_REG;//当 前设置为造区操作状态
		GCurShowState = SHOWSTATE_UNDEL;//设置当前为显示未删除状态
		this->Invalidate();
		GShowPnt = true;
		GShowReg = true;
...
void CMapEditorView::OnRegionMove()
{
	...
	if (GRegFCreated)
	{
		GCurOperState = OPERSTAE_MOVE_REG; // 设置当前为移动区操作状态
		GCurShowState = SHOWSTATE_UNDEL;//设置当前为显示未删除状态
		this->Invalidate();
		GShowPnt = true;
		GShowReg = true;
		...
		MessageBox(L"TempFile have not beencreated.", L"Message", MB_OK);
	}
}
...
void CMapEditorView::OnRegionDelete()
{
	...
	if (GRegFCreated)
	{
		GCurOperState = OPERSTAE_DELETE_REG;//设置当前为删除区操作状态
		GCurShowState = SHOWSTATE_UNDEL;//设置当前为显示未删除状态
		this->Invalidate();
		GShowPnt = true;
		GShowReg = true;
...
void CMapEditorView::OnRegionUndelete()
{
	...
	if (GRegFCreated)
	{
		GCurOperState = OPERSTATE_UNDELETE_REG;// 当前操作状态(恢复点)
		GCurShowState = SHOWSTATE_DEL;//当前显示状态(删除点)
		this->Invalidate();
...
void CMapEditorView::OnRegionModifyParameter()
{
	...
	if (GRegFCreated)
	{
		GCurOperState = OPERSTATE_MODIFY_REGION_PARAMETER; //修改点参数
		GCurShowState = SHOWSTATE_UNDEL; //显示状态(不删除点)
		GShowPnt = true;//打开显示点
		GShowLin = true;//关闭显示线
		GShowReg = true; //关闭显示区
...
void CMapEditorView::OnRegionSetDefparameter()
{
	...
	CRegionParameterDlg dlg; // 参数设置的对话框
	dlg.m_Pattern = GReg.pattern;//型
...
void CMapEditorView::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (GLinFCreated) {
		switch (GCurOperState) {
		case OPERSTAE_MOVE_LIN: //移动点操作
			D_DOT dot;
			PntToDot(dot, point);
			PntVPtoDP(dot, GZoom, GZoomOffset_x, GZoomOffset_y); //坐标系转换
			DotToPnt(point, dot);
			GLinMMTmpNdx = FindLin(GLinTmpNdxF, GLinTmpDatF, point, GLinNum, GLinNdx); // 查找单击点最近的一条线
			GLinMMOffsetX = 0;
			GLinMMOffsetY = 0;
			PntDPtoVP(dot, GZoom, GZoomOffset_x, GZoomOffset_y);//坐标系转换
			DotToPnt(point, dot);
	...
	if (GPntFCreated || GLinFCreated || GRegFCreated) {
		switch (GCurOperState) {
			case OPERSTATE_ZOOM_IN : // 当前为放大操作状态
				...
				GZoomMMPnt  = point;
				break;
				case OPERSTATE_WINDOW_MOVE ://当 前为移动操作状态
	...
	{
		switch (GCurOperState) {
			case OPERSTAE_MOVE_REG ://当 前为移动区操作状态
				GRegLBDPnt = point;
				GRegMMPnt = point;
				D_DOT dot;
				PntToDot(dot, point);
...
void CMapEditorView::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	if (GPntFCreated) //已创建临时文件
	{
		...
		switch (GCurOperState)
		{
			...
			case OPERSTAE_INPUT_PNT://当前为绘制点状态
				PNT_STRU pnt;//点对象
			...
			break;
			case OPERSTAE_MOVE_PNT :
			if (GPntNdx != -1) {
				PNT_STRU pnt;
				PntToDot(dot, point);
				PntVPtoDP(dot, GZoom, GZoomOffset_x, GZoomOffset_y); //坐 标系转换
			...
			break;
			case OPERSTATE_UNDELETE_PNT ://当 前操作状态(恢复点)
				PntToDot(dot, point);
	...
	if (GLinFCreated) //已创建临时文件
	{
		D_DOT dot;
		switch (GCurOperState) {
		case OPERSTAE_INPUT_LIN: // 当前为绘制线状态
			if (GTLin.dotNum == 0)
		...
		case OPERSTAE_DELETE_LIN:// 当前为删除线状态
			PntToDot(dot, point);
			PntVPtoDP(dot, GZoom, GZoomOffset_x, GZoomOffset_y); // 坐标系转换
			DotToPnt(point, dot);
			...
			break;
		case OPERSTAE_MOVE_LIN://当前为移动线操作状态
			if (GLinNdx != -1) {
		...
		case OPERSTATE_LINK_LIN://当前为连接线操作状态
			if (GnLine < 2) {
				...
				if (GnLine != 0) {
					...
					{
						A1terLindot(GLinTmpDatF, GStartLin, GEndLin, GnStart, GnEnd, GLin.datOff); //改变线的点数据,即将连接线的点写入文件中
		...
		case OPERSTATE_UNDELETE_LIN://当前操作状态(恢复线)
			PntToDot(dot, point);
			PntVPtoDP(dot, GZoom, GZoomOffset_x, GZoomOffset_y);//坐标系转换
			DotToPnt(point, dot);
			FindDeleteLin(GLinTmpNdxF, GLinTmpDatF, point, GLinNum, GLinNdx);//查找线
			if (GLinNdx != -1)
			{
				LIN_NDX_STRU TmpLinNdx;
				D_DOT dot1, dot2;
			...
			break;
		case OPERSTATE_MODIFY_LINE_PARAMETER: //修改参数操作状态
			PntToDot(dot, point);
			PntVPtoDP(dot, GZoom, GZoomOffset_x, GZoomOffset_y);//窗口转数据坐标
			DotToPnt(point, dot);
			LIN_NDX_STRU tempLine;
			memcpy_s(&tempLine, sizeof(LIN_NDX_STRU), &FindLin(GLinTmpNdxF, GLinTmpDatF, point,GLinNum,GLinNdx), sizeof(LIN_NDX_STRU)); // 查找最近点
			if (GLinNdx != -1) {
				CPointParameterDlg dlg;//点参数设置对话框.
				...
				if (IDOK == dlg.DoModal())
				{
					...
					GLinTmpNdxF->Write(&tempLine, sizeof(LIN_NDX_STRU));//写入数据
	...
	if (GRegFCreated) {
		...
		switch (GCurOperState)
		{
		   case OPERSTAE_INTPUT_REG ://当前为造区操作状态
				if (GTReg.dotNum == 0)
           ...
           case OPERSTAE_DELETE_REG ://当前为删除区操作状态
					PntToDot(dot, point);
					PntVPtoDP(dot, GZoom, GZoomOffset_x, GZoomOffset_y);//窗口转数据坐标
					DotToPnt(point, dot);
			 ...
			 case OPERSTATE_UNDELETE_REG:
				 ...
				 PntVPtoDP(dot, GZoom, GZoomOffset_x, GZoomOffset_y);//窗口转数据坐标
				 DotToPnt(point, dot);
				 FindDeleteReg(GRegTmpNdxF, GRegTmpDatF, point, GRegNum, GRegNdx);//查找
				 if (GRegNdx != -1)
				 {
					 GRegChanged = true;
					 REG_NDX_STRU TmpRegNdx;
					 ReadTempFileToRegNdx(GRegTmpNdxF, GRegNdx, TmpRegNdx);//从临时文件中读取删除区的索引
					 TmpRegNdx.isDel = 0;//设置未删除标记
					 UpdateReg(GRegTmpNdxF, GRegNdx, TmpRegNdx);//更新区数据
					 D_DOT* dot = new D_DOT[TmpRegNdx.dotNum];
					 GRegTmpDatF->Seek(TmpRegNdx.datOff, CFile::begin);
					 GRegTmpDatF->Read(dot, TmpRegNdx.dotNum * sizeof(D_DOT));
				 ...
				 break;
			 case OPERSTATE_MODIFY_REGION_PARAMETER: //修改参数操作状态
				 PntToDot(dot, point);
				 PntVPtoDP(dot, GZoom, GZoomOffset_x, GZoomOffset_y);//窗口转数据坐标
				 DotToPnt(point, dot);
				 REG_NDX_STRU tempRegion;
				 memcpy_s(&tempRegion, sizeof(REG_NDX_STRU), &FindReg(GRegTmpNdxF, GRegTmpDatF, point, GRegNum, GRegNdx), sizeof(REG_NDX_STRU)); // 查找最近点
				 if (GRegNdx != -1) {
					 CRegionParameterDlg dlg;//点参数设置对话框.
					 ...
					 if (IDOK == dlg.DoModal())
					 {
						 ...
						 GRegTmpNdxF->Write(&tempRegion, sizeof(REG_NDX_STRU));//写入数据
...
void CMapEditorView::OnDestroy()
{
	...
	// TODO: 在此处添加消息处理程序代码
	if (GPntFCreated) {//如果点临时文件已创建，则关闭删除
...
void CMapEditorView::OnMouseMove(UINT nFlags, CPoint point)
{
			...
			case OPERSTATE_ZOOM_IN :// 当前为放大操作状态
				...
				case OPERSTATE_WINDOW_MOVE : //当前为移动操作状态
					if (GWinMoveMMPnt.x != -1 && GWinMoveMMPnt.y != -1)
					{
						...
						GZoomOffset_x -= offset.x / GZoom; //变 换放大与缩小时的偏移量
	...
	if (GRegFCreated) {
		...
		case OPERSTAE_INTPUT_REG:
			...
			break;
			case OPERSTAE_MOVE_REG ://当前为移动区操作状态
				if (GRegNdx != -1)
				{
					CClientDC dc(this);
					dc.SetROP2(R2_NOTXORPEN); // 设置异或模式
...
void CMapEditorView::OnUpdatePointShowDelete(CCmdUI* pCmdUI)
{
	// TODO: 在此添加命令更新用户界面处理程序代码
...
void CMapEditorView::OnFileSaveRegion()
{
	...
	if (GRegFCreated) {
		...
		if (GRegFName.IsEmpty()) {
			...
			CFileDialog dlg(false, _T("reg"), NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, lpszFilters); //保存线的对话框
		...
		WriteTempToRegPermanentFile(RegF, GRegTmpDatF, GRegTmpNdxF, GRegVer, GRegNum, GRegLNum); // 将线的索引和点数据写入永久文件
		RegF->Close();
		delete RegF;
