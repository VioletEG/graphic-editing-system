...
void DrawPnt(CClientDC* dc, PNT_STRU point)
{
	...
	CObject *old0bject=nullptr;
	switch (point.pattern) 
	{
	case 0: old0bject = dc->SelectObject(&pen);
	...
	case 1: old0bject = dc ->SelectObject(&brush);
		dc->Ellipse((long)point.x - 2, (long)point.y - 2, (long)point.x + 2, (long)point.y + 2);
		break;

	case 2: old0bject = dc->SelectObject(&pen);//外部顶点，内部顶点
		
		POINT external_pt[5], interior_pt[5];//外部圆半径，内部圆半径
		
		double external_r = 10, interior_r = external_r / 2;//先顺时针求外部顶点，依次为正上方、右上方、右下方、左下方、左上方
	...
	dc->SelectObject(old0bject);
}

void ShowAl1Pnt(CClientDC* dc, CFile* pntTmpF, int pntNum)
{
	/*显示所有点*/
	PNT_STRU point;
